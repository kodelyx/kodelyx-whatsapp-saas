...
export default function ChatPage() {
  ...
  // Start from the SSR default (false) so server and first client render match,
  // then apply the persisted preference after mount to avoid a hydration mismatch.
  ...
  const { data: messages, error, isLoading, mutate: mutateMessages } = useSWR<Message[]>(swrKey, fetcher, { revalidateOnFocus: true, refreshInterval: 5000 });
  ...
  const filteredMessages = useMemo(() => {
    if (!messages) return [];
    // Deduplicate by message ID (Pusher + SWR poll can cause duplicates)
    ...
    const handleNewMessage = (payload: NewMessagePayload) => {
      ...
      if (payload.remoteJid === remoteJid && instanceMatch) {
        mutateMessages((currentMessages = []) => {
          ...
          return [...(currentMessages || []), messageWithStatus as unknown as Message];
        }, false);
