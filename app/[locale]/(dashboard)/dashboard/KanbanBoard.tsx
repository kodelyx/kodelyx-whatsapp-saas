...
export default function KanbanBoard() {
  ...
  const confirmDeleteStage = async () => {
    ...
    setIsProcessing(true);
    try {
        await fetch(`/api/funnel-stages/${deleteStageId}`, { method: 'DELETE' });
        toast.success(t('toast_deleted'));
        mutateStages(); mutateChats();
    } catch { toast.error(t('toast_error')); }
    finally { setIsProcessing(false); setDeleteStageId(null); }
  };
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden relative">
                        ...
                        <div className="flex h-full gap-4 pb-4 min-w-max px-1" ref={provided.innerRef} {...provided.droppableProps}>
                            ...
                            {stages.map((stage, index) => (
                                <Draggable key={stage.id} draggableId={stage.id.toString()} index={index}>
                                    {(provided) => (
                                        <div ref={provided.innerRef} {...(provided.draggableProps as any)} className="flex flex-col w-80 bg-muted/50 dark:bg-muted/20 rounded-xl border h-full max-h-full">
        ...
        <Dialog open={!!deleteStageId} onOpenChange={(o) => !o && setDeleteStageId(null)}>
            <DialogContent>
                <DialogHeader><DialogTitle>{t('delete_confirm')}</DialogTitle></DialogHeader>
                <DialogFooter>
                    <Button variant="outline" onClick={() => setDeleteStageId(null)} disabled={isProcessing}>Cancel</Button>
                    <Button variant="destructive" onClick={confirmDeleteStage} disabled={isProcessing}>
                        {isProcessing ? <Loader2 className="h-4 w-4 animate-spin"/> : t('delete')}
                    </Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
