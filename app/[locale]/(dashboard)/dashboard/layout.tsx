...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const { data: chats, error, isLoading, mutate: mutateChats } = useSWR<Chat[]>('/api/chats', fetcher, { refreshInterval: 5000 });
    ...
    const handleChatListUpdate = (updateData: ChatListUpdatePayload) => {
      ...
      mutate('/api/chats', async (currentChats: Chat[] | undefined = []) => {
        ...
        const existingIndex = chatsList.findIndex(c => {
          if (updateData.id != null && String(c.id) === String(updateData.id)) return true;
  ...
  const validChats = useMemo(() => {
    ...
    // Dedupe by id so duplicate entries (e.g. a realtime update that didn't match
    // an existing chat due to a string/number id mismatch) never collide as React keys.
    .filter((chat, index, list) => list.findIndex(c => String(c.id) === String(chat.id)) === index);
