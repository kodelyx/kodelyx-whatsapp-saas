...
import {
    ...
    CheckCircle2, XCircle, Clock, AlertTriangle, Eye, Smartphone, Trash2
} from 'lucide-react';
...
export default function TemplatesPage() {
    ...
    const handleDelete = async (template: WabaTemplate) => {
        try {
    ...
    return (
        <div className="flex flex-col h-full bg-muted p-6 overflow-hidden">
                        ...
                        filteredTemplates.map(tpl => (
                            <div key={tpl.id} className="grid grid-cols-12 gap-4 p-4 border-b last:border-0 hover:bg-muted transition-colors items-center group">
                                ...
                                <div className="col-span-2 flex justify-end gap-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
                                    ...
                                    <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-destructive" onClick={() => handleDelete(tpl)} title="Delete Template">
            ...
            <Dialog open={isViewOpen} onOpenChange={setIsViewOpen}>
                <DialogContent className="sm:max-w-[400px] p-0 bg-transparent border-none shadow-none flex justify-center items-center" aria-describedby="template-preview-desc">
                    <DialogTitle className="sr-only">Template Preview</DialogTitle>
                    <p id="template-preview-desc" className="sr-only">Preview of the selected WhatsApp template</p>
                    {selectedTemplate && (
                        <div className="transform scale-90 sm:scale-100 transition-transform w-full max-w-[340px]">
                            <div className="max-h-[70vh] overflow-y-auto rounded-2xl w-full">
