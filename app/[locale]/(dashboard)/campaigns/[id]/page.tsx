...
import { ArrowLeft, Loader2, CheckCircle, XCircle, Clock, Check, CheckCheck, Eye } from 'lucide-react';
...
function DeliveryBadge({ status }: { status?: string | null }) {
    if (status === 'read') return <span className="inline-flex items-center gap-1 text-sky-600 text-xs font-medium"><CheckCheck className="h-3.5 w-3.5" /> Read</span>;
    if (status === 'delivered') return <span className="inline-flex items-center gap-1 text-emerald-600 text-xs font-medium"><CheckCheck className="h-3.5 w-3.5" /> Delivered</span>;
    if (status === 'sent') return <span className="inline-flex items-center gap-1 text-muted-foreground text-xs font-medium"><Check className="h-3.5 w-3.5" /> Sent to WhatsApp</span>;
    if (status === 'failed') return <span className="inline-flex items-center gap-1 text-destructive text-xs font-medium"><XCircle className="h-3.5 w-3.5" /> Failed</span>;
...
export default function CampaignDetailsPage() {
    ...
    return (
        <div className="flex flex-col h-full bg-muted p-6">
            ...
            {campaign.delivery?.available && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                    <Card className="p-4 flex flex-col items-center justify-center bg-background border-emerald-500/20">
                        <span className="text-2xl font-bold text-emerald-600">{campaign.delivery.delivered}</span>
                        <span className="text-xs text-muted-foreground uppercase font-bold mt-1 flex items-center gap-1"><CheckCheck className="h-3 w-3" /> Delivered</span>
                    </Card>
                    <Card className="p-4 flex flex-col items-center justify-center bg-background border-sky-500/20">
                        <span className="text-2xl font-bold text-sky-600">{campaign.delivery.read}</span>
                        <span className="text-xs text-muted-foreground uppercase font-bold mt-1 flex items-center gap-1"><Eye className="h-3 w-3" /> Read</span>
                    </Card>
                    <Card className="p-4 flex flex-col items-center justify-center bg-background border-border">
                        <span className="text-2xl font-bold text-muted-foreground">{campaign.delivery.pending}</span>
                        <span className="text-xs text-muted-foreground uppercase font-bold mt-1 flex items-center gap-1"><Check className="h-3 w-3" /> Awaiting</span>
                    </Card>
                    <Card className="p-4 flex flex-col items-center justify-center bg-background border-destructive/20">
                        <span className="text-2xl font-bold text-destructive">{campaign.delivery.failed}</span>
                        <span className="text-xs text-muted-foreground uppercase font-bold mt-1 flex items-center gap-1"><XCircle className="h-3 w-3" /> Undelivered</span>
                    </Card>
                </div>
            )}
            ...
            <div className="flex-1 bg-background rounded-xl border border-border shadow-sm overflow-hidden flex flex-col">
                <div className="grid grid-cols-12 gap-4 p-4 border-b bg-muted/50 text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                    ...
                    <div className="col-span-4">{t('table_details')}</div>
                </div>
                    ...
                    {campaign.leads.map((lead: any) => (
                        <div key={lead.id} className="grid grid-cols-12 gap-4 p-4 border-b last:border-0 hover:bg-muted items-center">
                            ...
                            <div className="col-span-3">
                                <DeliveryBadge status={lead.deliveryStatus} />
                            </div>
                            <div className="col-span-4 text-xs font-mono break-all">
                                {lead.status === 'FAILED' ? (
