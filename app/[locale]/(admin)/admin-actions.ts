...
const planSchema = z.object({
  ...
  maxMonthlyMessages: z.coerce.number().min(0).default(0),
...
export async function upsertPlan(prevState: ActionState, formData: FormData): Promise<ActionState> {
  try {
    ...
    const rawData = {
      ...
      isMessagingEnabled: formData.get('isMessagingEnabled') === 'on',
      maxMonthlyMessages: formData.get('maxMonthlyMessages'),
