...
export async function POST(request: Request) {
  try {
    ...
    // Build body + header + url-button parameters; fail cleanly if a required
    // variable is missing instead of sending an empty param (Meta error 131008).
    const built = buildTemplateComponents(dbComponents, variables as Record<string, string> | null);
    if (built.error) {
    ...
    if (!response.ok) {
      ...
      console.error(`[Send Template] FAILED to ${metaPayload.to} | template=${template.name}:`, JSON.stringify(errorObj));
    ...
    console.log(`[Send Template] Sent to ${metaPayload.to} | template=${template.name} | msgId=${data.messages?.[0]?.id}`);
