...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(
      `https://graph.facebook.com/v25.0/${instance.metaBusinessId}/message_templates?fields=${fields}&limit=100`,
      {
    ...
    const upsertPromises = templates.map(async (tpl: any) => {
      ...
      // Keep original Meta CDN URLs for header images
      // (downloading locally causes truncation issues)
