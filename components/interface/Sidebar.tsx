...
/**
 * Shared sidebar navigation used by both the desktop rail (`Sidebar`) and the
 * mobile drawer in the dashboard layout.
 *
 * @param expanded  When true (drawer mode), labels and plan details always show.
 *                  When false (desktop), labels are hidden until the `lg` breakpoint
 *                  so the rail can collapse to icons on medium screens.
 * @param onNavigate Called when a link is tapped — used to close the mobile drawer.
 */
export function SidebarNav({ expanded = false, onNavigate }: { expanded?: boolean; onNavigate?: () => void }) {
  ...
  // Label visibility: always shown in the drawer, breakpoint-gated on desktop.
  const labelCls = expanded ? 'inline' : 'hidden lg:inline';
  const detailCls = expanded ? 'flex' : 'hidden lg:flex';
      ...
      <div className="p-2 border-t space-y-2">
        ...
        {isAdmin && (
          ...
          >
            ...
            <span className={labelCls}>Admin Panel</span>
          </Link>
        )}
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <div className={`${detailCls} flex-col min-w-0`}>
