...
describe('MetaCloudProvider', () => {
  ...
  it('should warn but not throw if metaToken is missing', () => {
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
    expect(() => new MetaCloudProvider({ ...mockConfig, metaToken: null })).not.toThrow();
    expect(warn).toHaveBeenCalled();
  });

  it('should warn but not throw if metaPhoneNumberId is missing', () => {
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
    expect(() => new MetaCloudProvider({ ...mockConfig, metaPhoneNumberId: null })).not.toThrow();
    expect(warn).toHaveBeenCalled();
  });
