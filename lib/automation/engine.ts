...
import { MetaCloudProvider } from '@/lib/plugins/meta-cloud/provider';
...
export async function processAutomation(
  ...
  if (currentNode.type === 'collect') {
      ...
      if (selectedEdge) {
          ...
          await sendAutomationMessage(config, remoteJid, "sendText", {
             text: fallbackMsg
          }, teamId, chatId);
          return true;
      }
...
async function executeStep(
    ...
    if (nextNode.type === 'delay') {
        ...
        await sendAutomationMessage(instance, remoteJid, "sendText", { text: text }, teamId, chatId);
        ...
        await sendAutomationMessage(instance, remoteJid, "sendText", { text: finalMsg }, teamId, chatId);
        _logNode({ url });
        await moveToNextAuto(updatedSession, flow, nextNode.id, instance, remoteJid, teamId, chatId);
    }
...
async function processTextOutput(node: Node, instance: InstanceConfig, remoteJid: string, teamId: number, chatId: number, variables: Record<string, any>) {
    if (node.type === 'message') {
        ...
        text = replaceVariables(text, variables);
        await sendAutomationMessage(instance, remoteJid, "sendText", { text }, teamId, chatId);
    }
        ...
        await sendAutomationMessage(instance, remoteJid, "sendText", { text: message }, teamId, chatId);
    }
}
...
async function processMediaOutput(node: Node, instance: InstanceConfig, remoteJid: string, teamId: number, chatId: number, variables: Record<string, any> = {}) {
    ...
    try {
        ...
        await sendAutomationMessage(instance, remoteJid, "sendMedia", payload, teamId, chatId, data.mediaUrl);
...
async function sendAutomationMessage(
    ...
    try {
        ...
        const result = endpoint === 'sendText'
            ? await provider.sendText(remoteJid, { text: contentPayload.text })
            : await provider.sendMedia(remoteJid, {
                mediaBase64: contentPayload.media,
                mimetype: contentPayload.mimetype,
                mediaType: contentPayload.mediatype,
