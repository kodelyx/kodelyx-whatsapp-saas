...
async function sendAiMedia(
    ...
    if (!instance.metaToken || !instance.metaPhoneNumberId) return { error: 'No Meta credentials' };
    ...
    const to = remoteJid.replace(/@s\.whatsapp\.net$/, '').replace(/@g\.us$/, '');

    // Meta Cloud accepts media by public link. Audio does not support captions.
    const mediaObject: any = { link: finalMediaUrl };
    if (type === 'document') {
        mediaObject.filename = 'file';
        if (caption) mediaObject.caption = caption;
    } else if (type !== 'audio' && caption) {
    ...
    try {
        const response = await fetch(`https://graph.facebook.com/v21.0/${instance.metaPhoneNumberId}/messages`, {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Meta Cloud API failed: ${response.status}`);
        }
