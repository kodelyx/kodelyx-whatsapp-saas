...
export const autoReplySettings = pgTable('auto_reply_settings', {
  ...
  autoReplyMessage: text('auto_reply_message').default('Thank you for contacting us! We will get back to you shortly.'),
  autoReplyDelaySeconds: integer('auto_reply_delay_seconds').default(300),
  autoReplyIntervalHours: integer('auto_reply_interval_hours').default(12),

  // Follow-Up #1
  followup1Enabled: boolean('followup1_enabled').default(false).notNull(),
  followup1Message: text('followup1_message').default('🔔 Reminder: Check our latest offers!'),
  followup1DelayMinutes: integer('followup1_delay_minutes').default(480),

  // Follow-Up #2
  followup2Enabled: boolean('followup2_enabled').default(false).notNull(),
  followup2Message: text('followup2_message').default('⏰ Last chance! This offer expires soon.'),
  followup2DelayMinutes: integer('followup2_delay_minutes').default(720),

  createdAt: timestamp('created_at').notNull().defaultNow(),
  updatedAt: timestamp('updated_at').notNull().defaultNow(),
}, (table) => ({
  uniqueInstanceSettings: unique('auto_reply_instance_idx').on(table.teamId, table.instanceId),
}));

// Auto-Reply Cooldown Logs (prevent spam to same number)
export const autoReplyLogs = pgTable('auto_reply_logs', {
  ...
  uniquePhoneInstance: unique('auto_reply_phone_instance_idx').on(table.instanceId, table.phone),
}));
...
export const autoReplySettingsRelations = relations(autoReplySettings, ({ one }) => ({
  team: one(teams, { fields: [autoReplySettings.teamId], references: [teams.id] }),
  instance: one(evolutionInstances, { fields: [autoReplySettings.instanceId], references: [evolutionInstances.id] }),
}));

export const autoReplyLogsRelations = relations(autoReplyLogs, ({ one }) => ({
  team: one(teams, { fields: [autoReplyLogs.teamId], references: [teams.id] }),
  instance: one(evolutionInstances, { fields: [autoReplyLogs.instanceId], references: [evolutionInstances.id] }),
}));

export const scheduledFollowupsRelations = relations(scheduledFollowups, ({ one }) => ({
  team: one(teams, { fields: [scheduledFollowups.teamId], references: [teams.id] }),
  instance: one(evolutionInstances, { fields: [scheduledFollowups.instanceId], references: [evolutionInstances.id] }),
}));
...
export const teamsRelations = relations(teams, ({ one, many }) => ({
  ...
  messageCredits: one(messageCredits),
  messageCreditTransactions: many(messageCreditTransactions),
}));
...
export const branding = pgTable('branding', {
  id: serial('id').primaryKey(),
  name: varchar('name', { length: 100 }).notNull().default('Kodelyx'),
...
export const messageCredits = pgTable('message_credits', {
  ...
  teamIdUnique: unique('message_credits_team_id_idx').on(table.teamId),
}));

export const messageCreditTransactions = pgTable('message_credit_transactions', {
  id: serial('id').primaryKey(),
  teamId: integer('team_id')
    .notNull()
    .references(() => teams.id, { onDelete: 'cascade' }),
  type: varchar('type', { length: 20 }).notNull(), // 'purchase' | 'deduct' | 'bonus' | 'offline_request'
  amount: integer('amount').notNull(),
  description: text('description'),
  status: varchar('status', { length: 20 }).notNull().default('completed'),
  utr: varchar('utr', { length: 50 }),
  ...
  teamIdIdx: index('msg_credit_tx_team_id_idx').on(table.teamId),
}));

export const messageCreditsRelations = relations(messageCredits, ({ one }) => ({
  team: one(teams, {
    fields: [messageCredits.teamId],
    references: [teams.id],
  }),
}));

export const messageCreditTransactionsRelations = relations(messageCreditTransactions, ({ one }) => ({
  team: one(teams, {
    fields: [messageCreditTransactions.teamId],
    references: [teams.id],
  }),
}));
...
export type MessageCredits = typeof messageCredits.$inferSelect;
export type NewMessageCredits = typeof messageCredits.$inferInsert;
export type MessageCreditTransaction = typeof messageCreditTransactions.$inferSelect;
export type NewMessageCreditTransaction = typeof messageCreditTransactions.$inferInsert;
